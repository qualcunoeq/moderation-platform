...
// Definisci un tipo per le categorie di moderazione di OpenAI.
// Questo è basato su ciò che OpenAI restituisce in `result.categories`.
...
export async function POST(request: NextRequest) {
    try {
        ...
        return NextResponse.json({
            ...
            // Modifica qui: aggiunge l'asserzione di tipo 'as keyof Categories'
            categories: Object.keys(result.categories).filter(
                (key) => result.categories[key as keyof Categories]
